...
a sortable table (click a header), an Excel-style column filter and a
date-range filter, status filter cards, a search box, click-to-edit cells
(double-click a value, Esc cancels, no page reload), and bulk actions. It
reuses Celerp's own table JavaScript so the behavior matches the rest of the app.
...
acme-maintenance/               the module (copy and rename this whole folder)
  ...
  acme_maintenance/             the inner Python package (underscore, not hyphen)
    models.py                   one table: company equipment (UUID + company_id)
    routes.py                   API: list / add / edit-field / mark-serviced / delete
    ui_routes.py                the /maintenance page: sortable table, filters,
